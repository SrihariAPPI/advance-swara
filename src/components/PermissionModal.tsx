...
export default function PermissionModal({ onClose, errorMessage }: Props) {
  const isApiKeyError = errorMessage?.includes("API_KEY") || errorMessage?.includes("API key");
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
      ...
      >
        ...
        <h2 className="text-2xl font-serif font-medium text-marigold mb-3">
          {isApiKeyError ? "Configuration Required" : "Microphone Blocked"}
        </h2>
        ...
        <div className="flex flex-col w-full gap-3">
          ...
          <button 
            onClick={onClose}
            className={`w-full py-3 px-4 ${isApiKeyError ? 'bg-marigold text-peacock' : 'bg-cream/5 text-cream/70'} font-medium rounded-xl hover:${isApiKeyError ? 'bg-saffron' : 'bg-cream/10'} transition-colors`}
          >
            {isApiKeyError ? "Close & Open Settings" : "Close"}
          </button>
        </div>
      </motion.div>
    </div>
