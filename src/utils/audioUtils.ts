export async function playPCM(base64Data: string): Promise<void> {
  try {
    ...
    try {
      ...
      const pcmCtx = new AudioContextClass({ sampleRate: 24000 });
      const buffer = new Int16Array(bytes.buffer);
      const audioBuffer = pcmCtx.createBuffer(1, buffer.length, 24000);
      const channelData = audioBuffer.getChannelData(0);
      for (let i = 0; i < buffer.length; i++) {
        channelData[i] = buffer[i] / 32768.0;
      }
      const source = pcmCtx.createBufferSource();
      source.buffer = audioBuffer;
      source.connect(pcmCtx.destination);
      source.start();
